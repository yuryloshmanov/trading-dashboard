...
extension Data {
    func hmac(base64key key: String) -> String {
        ...
        key.withCString { keyPtr in
            self.withUnsafeBytes { (dataPtr: UnsafeRawBufferPointer) in
                CCHmac(algorithm, keyPtr, keyLength, dataPtr.baseAddress!, self.count, &output)
            }
        }

        let result = output.map { b in
                String(format: "%02x", b)
            }
            .joined()
        return result
...
public struct BinanceApi {
    static let baseUrl = URL(string: "https://fapi/binance.com/")!
    ...
    public init(apiKey: String?, secretKey: String?, receiveWindow: TimeInterval? = nil) {
        self.session = BinanceApi.defaultSession
    }
...
//        var defaultHeaders = Alamofire.Session.defaultHTTPHeaders
        var defaultHeaders = AF.session.configuration.headers.dictionary
        ...
        let session = Alamofire.Session(configuration: configuration, interceptor: interceptor)
        return session
    }

    @discardableResult
    public func send<T: BinanceRequest>(
            _ request: T, completionHandler: ((DataResponse<T.Response, Alamofire.AFError>) -> Void)?
    ) -> DataRequest {
...
public struct BinanceRequestInterceptor: RequestInterceptor {
    let apiKey: String?
    ...
    public func adapt(_ urlRequest: URLRequest, for session: Session, completion: @escaping (Swift.Result<URLRequest, Error>) -> Void) {
        guard let url = urlRequest.url else {
            completion(.failure(BinanceApiError.unknown))
            return
        }
        ...
        guard let apiKey = self.apiKey else {
            completion(.failure(BinanceApiError.noApiKeySpecified))
            return
        }
        guard let secretKey = self.secretKey else {
            completion(.failure(BinanceApiError.noApiKeySpecified))
            return
        }
...
extension BinanceOrder: CustomStringConvertible {
    public var description: String {
        return "BinanceOrder(\(symbol), \(orderId), \(clientOrderId))"
    }
}
