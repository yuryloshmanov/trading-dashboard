...
//    private static func BinanceRequestResponseSerializer<T: Decodable>(_ keyPath: String?, _ decoder: JSONDecoder) -> DataResponseSerializer {
//        return DataResponseSerializer {
//            (request: URLRequest?, response: HTTPURLResponse?, data: Data?, error: Error?) -> Result<T, Error> in
//
//            if let error = error {
...
//    }
//
//    private static func decodeToObject<T: Decodable>(_ type: T.Type, decoder: JSONDecoder, response: HTTPURLResponse?, data: Data?) -> Result<T, Error> {
...
//    }
//
//    private static func decodeToObject<T: Decodable>(byKeyPath keyPath: String, decoder: JSONDecoder, response: HTTPURLResponse?, data: Data?) -> Result<T, BinanceApiError> {
...
extension Encodable {
    ...
    var asParameters: Parameters? {
        ...
        try? JSONSerialization.jsonObject(with: self.asData, options: .allowFragments)
        else {
            return nil
        }
